...
class Asset(models.Model):
    ...
    TRANSICIONES_VALIDAS = {
        ...
        EstadoChoices.ASIGNADO: [EstadoChoices.PENDIENTE_DEVOLUCION, EstadoChoices.DADO_DE_BAJA],
        EstadoChoices.PENDIENTE_DEVOLUCION: [
            EstadoChoices.DISPONIBLE, EstadoChoices.EN_REVISION, EstadoChoices.DADO_DE_BAJA,
        ],
        EstadoChoices.EN_REVISION: [EstadoChoices.DISPONIBLE, EstadoChoices.DADO_DE_BAJA],
    ...
    tipo = models.ForeignKey(
        ...
        related_name='activos', verbose_name='Tipo'
    )
...
class AssetAssignment(models.Model):
    ...
    estado_devolucion = models.CharField(
        max_length=15, choices=EstadoDevolucionChoices.choices,
        blank=True, null=True, verbose_name='Estado de devolución'
    )
    foto_evidencia = models.CharField(
        max_length=500, blank=True, null=True,
        verbose_name='Foto evidencia (ruta/URL)'
    )
    activo = models.ForeignKey(
        Asset, on_delete=models.CASCADE,
        related_name='asignaciones', verbose_name='Activo'
    )
    trabajador = models.ForeignKey(
        'workers.Worker', on_delete=models.CASCADE,
        related_name='activos_asignados', verbose_name='Trabajador'
    )
    ...
    def clean(self):
        ...
        if self.fecha_devolucion and self.fecha_asignacion and \
           self.fecha_devolucion < self.fecha_asignacion:
            raise ValidationError(
