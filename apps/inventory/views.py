    ...
    def get_queryset(self):
        ...
        if not user.roles.filter(nombre__in=ROLES_INVENTARIO).exists():
            if user.roles.filter(nombre="ti").exists():
                tipos_ti = AssetType.objects.filter(es_ti=True).values_list(
                    "pk", flat=True
                )
                qs = qs.filter(tipo__in=tipos_ti)
            elif user.roles.filter(nombre="prevencion").exists():
                tipos_epp = AssetType.objects.filter(es_prevencion=True).values_list(
                    "pk", flat=True
                )
...
class AssetDetailView(RoleRequiredMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ctx = super().get_context_data(**kwargs)
        ctx["asignaciones"] = self.object.asignaciones.select_related(
            "trabajador"
        ).all()
    ...
    def post(self, request, pk):
        ...
        if not request.user.roles.filter(nombre__in=ROLES_INVENTARIO).exists():
            ...
            if "ti" in roles_usuario and not asset.tipo.es_ti:
                messages.error(
                    request, "Solo puedes asignar activos gestionados por TI."
                )
        ...
        messages.success(request, f"Activo {asset.codigo} asignado a {worker.nombre}.")
        return redirect("inventory:asset_detail", pk=pk)
    ...
    def post(self, request, pk):
        ...
        if not request.user.roles.filter(nombre__in=ROLES_INVENTARIO).exists():
            ...
            if "ti" in roles_usuario and not asset.tipo.es_ti:
                messages.error(
                    request,
                    "Solo puedes gestionar devoluciones de activos gestionados por TI.",
                )
                return redirect("inventory:asset_detail", pk=pk)
            if "prevencion" in roles_usuario and not asset.tipo.es_prevencion:
                messages.error(
                    request, "Solo puedes gestionar devoluciones de activos de EPP."
                )
                return redirect("inventory:asset_detail", pk=pk)
        ...
        if not asignacion_id or not estado_dev:
            messages.error(
                request, "Debe seleccionar la asignación y el estado de devolución."
            )
            return redirect("inventory:asset_detail", pk=pk)

        if estado_dev in ("danado", "con_perdida") and not foto_evidencia:
            messages.error(
                request, "Debe agregar foto como evidencia del daño/pérdida."
            )
            return redirect("inventory:asset_detail", pk=pk)
        ...
        asignacion.save(
            update_fields=["fecha_devolucion", "estado_devolucion", "foto_evidencia"]
        )
