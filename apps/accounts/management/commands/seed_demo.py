...
from apps.inventory.models import AssetType, Asset
from apps.processes.services import (
    crear_proceso_contratacion,
    crear_proceso_cambio_ceco,
...
class Command(BaseCommand):
        ...
        for run, nombre, correo, cargo, ceco in workers_data:
            w, _ = Worker.objects.get_or_create(
                ...
                defaults={
                    ...
                    "fecha_ingreso_efectiva": timezone.now().date()
                    - timedelta(days=90),
                },
            )
            workers.append(w)
    ...
    def _crear_activos(self, cecos, workers, admin):
        ...
        epp_models = [
            ...
            "Bota seguridad 3M",
            "Zapato dieléctrico",
            "Bota punta composite",
            "Lente 3M claro",
            "Lente oscuro",
            "Goggle antiempañante",
            "Arnés cuerpo completo",
            "Arnés posicionamiento",
            "Orejera 3M Peltor",
            "Tapón reutilizable",
            "Respirador N95",
            "Media cara 3M",
