...
class CustomLoginView(LoginView):
    ...
    def form_valid(self, form):
        ...
        if not user.is_active:
            messages.error(self.request, "Cuenta bloqueada. Contacte a RRHH.")
        ...
        if user.contrasena_temporal:
            ...
            messages.warning(
                self.request, "Debes cambiar tu contraseña temporal antes de continuar."
            )
        ...
        if email:
            try:
                ...
                if intentos >= 3:
                    ...
                    messages.warning(self.request, f"Intento fallido #{intentos}.")
...
class UserCreateView(RoleRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        messages.success(
            self.request, f"Usuario {self.object.email} creado exitosamente."
        )
        return response
