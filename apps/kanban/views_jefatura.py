...
class JefaturaNominaView(RoleRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        for ceco in cecos:
            ceco.workers_activos = (
                Worker.objects.filter(centro_costo_actual=ceco)
        ...
        if ceco_seleccionado:
            workers = (
                Worker.objects.filter(centro_costo_actual=ceco_seleccionado)
                .exclude(estado=Worker.EstadoChoices.ELIMINADO)
                .order_by("nombre")
            )
        else:
...
class JefaturaTrabajadorDetailView(RoleRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if worker.fecha_termino_contrato:
            worker.dias_restantes_contrato = (
                worker.fecha_termino_contrato - date.today()
            ).days
        ...
        procesos_activos = (
            Process.objects.filter(
                trabajador=worker, estado=Process.EstadoChoices.EN_CURSO
            )
        ...
        historial = (
            Process.objects.filter(trabajador=worker)
            .select_related("usuario_inicio")
...
class JefaturaProcesosView(RoleRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        procesos = Process.objects.filter(
            trabajador__in=workers_ceco, estado=Process.EstadoChoices.EN_CURSO
...
class JefaturaCeCoView(RoleRequiredMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if ceco:
            workers = Worker.objects.filter(centro_costo_actual=ceco).exclude(
                estado=Worker.EstadoChoices.ELIMINADO
            )

            ctx["total_workers"] = workers.count()
            ctx["workers_activos"] = workers.filter(
                estado=Worker.EstadoChoices.ACTIVO
            ).count()
            ctx["workers_en_transito"] = workers.filter(
                estado=Worker.EstadoChoices.EN_TRANSITO
            ).count()
            ctx["workers_por_egresar"] = workers.filter(
                estado=Worker.EstadoChoices.POR_EGRESAR
            ).count()

            ctx["procesos_activos"] = Process.objects.filter(
                trabajador__in=workers, estado=Process.EstadoChoices.EN_CURSO
            ).count()
