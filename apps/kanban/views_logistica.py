...
class LogisticaDashboardView(RoleRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        activos_asignados = Asset.objects.filter(estado=Asset.EstadoChoices.ASIGNADO).count()
        activos_disponibles = Asset.objects.filter(estado=Asset.EstadoChoices.DISPONIBLE).count()
        ...
        devoluciones_pendientes = AssetAssignment.objects.filter(
            ...
            proceso__tipo__in=[Process.TipoChoices.TERMINO, Process.TipoChoices.DESPIDO],
            proceso__estado=Process.EstadoChoices.EN_CURSO,
        ).count()
...
class LogisticaRegistrarDevolucionView(RoleRequiredMixin, View):
    ...
    def post(self, request, pk):
        ...
        if estado_devolucion not in dict(AssetAssignment.EstadoDevolucionChoices.choices):
