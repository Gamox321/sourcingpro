...
class RRHHDashboardView(RoleRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        workers_activos = Worker.objects.filter(estado=Worker.EstadoChoices.ACTIVO).count()
        
        procesos_activos = Process.objects.filter(estado=Process.EstadoChoices.EN_CURSO).count()
    ...
    def _get_carga_por_area(self):
        ...
        for area in AREA_ORDER:
            count = activas.filter(tipo__in=[
                k for k, v in Task.TIPO_AREA_MAP.items() if v == area
            ]).count()
            resultado[area] = {
...
class RRHHReportesView(RoleRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        total_workers = Worker.objects.exclude(estado=Worker.EstadoChoices.ELIMINADO).count()
        workers_activos = Worker.objects.filter(estado=Worker.EstadoChoices.ACTIVO).count()
        workers_en_proceso = Worker.objects.filter(estado=Worker.EstadoChoices.EN_PROCESO).count()
        workers_desvinculados = Worker.objects.filter(estado=Worker.EstadoChoices.DESVINCULADO).count()
        ...
        distribucion_ceco = Worker.objects.filter(
            estado=Worker.EstadoChoices.ACTIVO,
            centro_costo_actual__isnull=False,
        ).values(
            'centro_costo_actual__nombre'
        ).annotate(
            cantidad=Count('id')
        ...
        procesos_por_tipo = Process.objects.filter(
...
class RRHHConfigPlazosView(RoleRequiredMixin, TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        config.plazo_escalamiento_dias = int(request.POST.get('plazo_escalamiento_dias', config.plazo_escalamiento_dias))
        config.es_critica = request.POST.get('es_critica') == 'on'
        
        try:
            config.full_clean()
            config.save()
            messages.success(request, f'Plazo actualizado para {config.get_tipo_tarea_display()}.')
...
class RRHHAlertasContratosView(RoleRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        ).select_related('centro_costo_actual').order_by('fecha_termino_contrato')
...
class RRHHConfirmarCierreTerminoView(RoleRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        proceso = get_object_or_404(Process, pk=proceso_id, tipo=Process.TipoChoices.TERMINO)
        
        tareas = proceso.tareas.all()
        todas_completas = tareas.filter(
            estado__in=[Task.EstadoChoices.COMPLETADA, Task.EstadoChoices.GESTIONADO_EXTERNO]
        ).count() == tareas.count()
    ...
    def post(self, request, *args, **kwargs):
        ...
        messages.success(request, f'Término de contrato de {proceso.trabajador.nombre} confirmado.')
...
class RRHHProcesoDetailView(RoleRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ctx = super().get_context_data(**kwargs)
        proceso_id = self.kwargs.get('pk')
        proceso = get_object_or_404(Process.objects.select_related('trabajador', 'usuario_inicio', 'ceco_origen', 'ceco_destino'), pk=proceso_id)
        ...
        if proceso.tipo == Process.TipoChoices.CAMBIO_CECO:
            historial_ceco = proceso.trabajador.historial_ceco.select_related('centro_costo').all()
