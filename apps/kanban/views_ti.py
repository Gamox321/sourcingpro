...
class TIDashboardView(RoleRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        tipos_ti = AssetType.objects.filter(es_ti=True, estado="activo").values_list(
            "pk", flat=True
        )
        if tipos_ti:
        ...
        if tipos_ti:
            inventario_ti = (
                Asset.objects.filter(tipo__in=tipos_ti)
                .select_related("tipo")
                .prefetch_related("asignaciones__trabajador")[:10]
            )
        else:
...
class TIBloqueoUrgenteView(RoleRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        bloqueos_urgentes = (
            ...
            .select_related("proceso__trabajador", "usuario_responsable")
            .order_by("proceso__fecha_inicio")
        )
...
class TIConfirmarBloqueoView(RoleRequiredMixin, View):
    ...
    def post(self, request, pk):
        ...
        messages.success(
            request,
            f"Bloqueo de {task.proceso.trabajador.nombre} confirmado exitosamente.",
        )
        return redirect("ti:bloqueo_urgente")
