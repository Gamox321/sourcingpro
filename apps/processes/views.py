...
class ProcessCreateContratacionView(RoleRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx["descripcion"] = (
            "Registra un nuevo trabajador e inicia el proceso de contratacion con todas las areas involucradas."
        )
        ctx["tareas_info"] = [
    ...
    def post(self, request):
        ...
        if form.is_valid():
            try:
                ...
                messages.success(
                    request, "Proceso de contratacion iniciado exitosamente."
                )
...
class ProcessCreateCambioCeCoView(RoleRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx["descripcion"] = (
            "Transfere un trabajador activo de un centro de costo a otro, "
...
class ProcessCreateTerminoView(RoleRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx["descripcion"] = (
            "Gestiona la salida programada de un trabajador: devolucion de activos, finiquito y bloqueo de accesos."
        )
        ctx["tareas_info"] = [
...
class ProcessCreateDespidoView(RoleRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx["descripcion"] = (
            "Inicia un proceso de despido con bloqueo inmediato de accesos, "
...
class ProcessCreateAsignacionActivosView(RoleRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ctx = super().get_context_data(**kwargs)
        ctx["form"] = AsignacionActivosForm(
            user_cecos=_get_user_cecos(self.request.user)
        )
        ctx["tipo"] = "asignacion_activos"
        ctx["titulo"] = "Asignacion de Activos TI"
        ctx["descripcion"] = (
            "Solicita la asignacion de equipo TI a un trabajador activo. "
            "El area de TI seleccionara y asignara el equipo correspondiente."
        )
        ctx["tareas_info"] = [
    ...
    def post(self, request):
        form = AsignacionActivosForm(
            request.POST, user_cecos=_get_user_cecos(request.user)
        )
        if form.is_valid():
            try:
...
class ProcessDetailView(RoleRequiredMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        completadas = sum(
            ...
            for t in tareas
            if t.estado
            in (Task.EstadoChoices.COMPLETADA, Task.EstadoChoices.GESTIONADO_EXTERNO)
        )
        total = tareas.count()
        ctx["tareas_completadas_count"] = completadas
        ctx["porcentaje_completado"] = (
            int(completadas / total * 100) if total > 0 else 0
        )
...
class TaskCompleteView(RoleRequiredMixin, View):
    ...
    def post(self, request, pk, task_pk):
        ...
        if anteriores.exists():
            ...
            messages.error(
                request, f"No puede completar esta tarea. Primero complete: {names}."
            )
            return redirect("processes:process_detail", pk=pk)
        ...
        if (
            ...
            if finiquito and finiquito.estado != Task.EstadoChoices.COMPLETADA:
                messages.error(
                    request,
                    "No puede bloquear accesos hasta que Finanzas complete la "
                    "coordinación de finiquito. "
                    f'<a href="mailto:?subject=Recordatorio: '
                    f'Finiquito proceso #{task.proceso.pk}" '
                    f'class="alert-link">Enviar recordatorio a Finanzas</a>',
                )
                return redirect("processes:process_detail", pk=pk)
    ...
    def post(self, request, pk, task_pk):
        ...
        if email:
            ...
            worker.save(
                update_fields=[
                    "cuenta_ti_email",
                    "cuenta_ti_clave_inicial",
                    "cuenta_ti_fecha_creacion",
                    "cuenta_ti_notas",
                ]
            )
        ...
        if email:
            ...
            messages.success(request, "Tarea completada (sin registrar credenciales).")
