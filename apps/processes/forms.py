...
class ContratacionForm(forms.Form):
    ...
    nombre = forms.CharField(max_length=100, label='Nombre completo',
                             widget=forms.TextInput(attrs={'class': 'form-control', 'placeholder': 'Juan Perez Garcia'}))
    correo = forms.EmailField(label='Correo electronico',
                              widget=forms.EmailInput(attrs={'class': 'form-control', 'placeholder': 'juan.perez@empresa.cl'}))
    cargo = forms.CharField(max_length=100, label='Cargo',
                            widget=forms.TextInput(attrs={'class': 'form-control', 'placeholder': 'Analista de Sistemas'}))
    ...
    def __init__(self, *args, **kwargs):
        ...
        if user_cecos is not None:
            self.fields['centro_costo'].queryset = self.fields['centro_costo'].queryset.filter(
                pk__in=user_cecos.values_list('pk', flat=True)
            )

    def clean_run(self):
        run = self.cleaned_data.get('run', '').strip()
        if not RUN_RE.match(run):
            raise ValidationError('Formato RUN invalido. Use 12345678-9 (sin puntos, con guion y digito verificador).')
...
class CambioCeCoForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        super().__init__(*args, **kwargs)
        if user_cecos is not None:
            self.fields['ceco_destino'].queryset = self.fields['ceco_destino'].queryset.filter(
                pk__in=user_cecos.values_list('pk', flat=True)
            )
    ...
    def clean_trabajador(self):
        ...
        if worker and worker.estado != Worker.EstadoChoices.ACTIVO:
            raise ValidationError(f'El trabajador esta en estado "{worker.get_estado_display()}". Solo trabajadores activos pueden cambiar de CeCo.')
        if worker and Process.objects.filter(
            trabajador=worker, tipo=Process.TipoChoices.CAMBIO_CECO, estado=Process.EstadoChoices.EN_CURSO
        ).exists():
            raise ValidationError('Este trabajador ya tiene un cambio de CeCo en curso.')
        return worker
...
class TerminoForm(forms.Form):
    ...
    fecha_termino = forms.DateField(label='Fecha de termino',
                                    widget=forms.DateInput(attrs={'class': 'form-control', 'type': 'date'}))
    motivo = forms.CharField(label='Motivo',
                             widget=forms.Textarea(attrs={'class': 'form-control', 'rows': 2}))
    ...
    def clean_trabajador(self):
        worker = self.cleaned_data.get('trabajador')
        if worker and Process.objects.filter(
            trabajador=worker, tipo=Process.TipoChoices.TERMINO, estado=Process.EstadoChoices.EN_CURSO
        ).exists():
            raise ValidationError('Este trabajador ya tiene un proceso de termino en curso.')
        return worker
...
class DespidoForm(forms.Form):
    ...
    def clean_trabajador(self):
        worker = self.cleaned_data.get('trabajador')
        if worker and Process.objects.filter(
            trabajador=worker, tipo=Process.TipoChoices.DESPIDO, estado=Process.EstadoChoices.EN_CURSO
        ).exists():
