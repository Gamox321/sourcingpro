...
def _find_user_for_role(role_name):
    user = (
        User.objects.filter(is_active=True, roles__nombre=role_name).distinct().first()
    )
    if user is None:
        logger.warning(
...
def _create_task(
    ...
    AuditLog.objects.create(
        ...
        valor_nuevo={
            "tipo": tipo,
            "urgencia": urgencia,
            "responsable": responsable.email if responsable else None,
        },
        usuario=proceso.usuario_inicio,
...
def crear_proceso_termino(usuario, worker_id, fecha_termino, motivo):
    ...
    _create_task(
        proceso,
        Task.TipoChoices.FINIQUITO_COORDINACION,
...
def crear_proceso_despido(usuario, worker_id, fecha, motivo, causal_legal):
    ...
    if ti_user:
        notificar(
            ...
            titulo="Despido iniciado — Bloqueo inmediato requerido",
    ...
    if finanzas_user:
        notificar(
            ...
            tipo_evento="proceso_despido_inicio",
            titulo="Despido iniciado — Coordinar finiquito",
            descripcion=f"Se ha iniciado proceso de despido para {worker.nombre}. "
    ...
    try:
        logistica_users = User.objects.filter(
            roles__nombre=Role.RoleChoices.LOGISTICA, is_active=True
        ).distinct()
...
def crear_proceso_asignacion_activos(usuario, worker_id, comentario=""):
    ...
    _notificar_iniciador(
        proceso,
        "proceso_inicio",
        "Asignacion de Activos TI iniciada",
...
def completar_tarea(task):
    ...
    AuditLog.objects.create(
        ...
        valor_nuevo={
            ...
            "fecha_completado": str(task.fecha_completado),
        },
...
def _finalizar_proceso(proceso):
    ...
    AuditLog.objects.create(
        ...
        valor_nuevo={
            ...
            "fecha_cierre": str(proceso.fecha_cierre),
        },
        usuario=proceso.usuario_inicio,
        id_entidad_afectada=proceso.pk,
    )
...
def cancelar_proceso(proceso, motivo_cancelacion=None):
    ...
    worker.save(
        ...
        else ["estado", "centro_costo_actual"]
    )
