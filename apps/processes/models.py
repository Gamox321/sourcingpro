...
class Process(models.Model):
    ...
    trabajador = models.ForeignKey(
        ...
        related_name='procesos', verbose_name='Trabajador'
    )
    usuario_inicio = models.ForeignKey(
        'accounts.User', on_delete=models.CASCADE,
        related_name='procesos_iniciados', verbose_name='Iniciado por'
    )
    ceco_origen = models.ForeignKey(
        'clients.CostCenter', on_delete=models.SET_NULL,
        blank=True, null=True, related_name='procesos_origen',
        verbose_name='CeCo origen'
    )
    ceco_destino = models.ForeignKey(
        'clients.CostCenter', on_delete=models.SET_NULL,
        blank=True, null=True, related_name='procesos_destino',
    ...
    def clean(self):
        if self.causal_legal and self.tipo != self.TipoChoices.DESPIDO:
            raise ValidationError({'causal_legal': 'Solo aplica en procesos de despido.'})
        if self.ceco_destino and self.tipo != self.TipoChoices.CAMBIO_CECO:
            raise ValidationError({'ceco_destino': 'Solo aplica en cambios de centro de costo.'})
...
class Task(models.Model):
    class TipoChoices(models.TextChoices):
        ...
        EXAMENES_PREOCUPACIONALES = 'examenes_preocupacionales', 'Exámenes preocupacionales'
    ...
    tipo = models.CharField(max_length=30, choices=TipoChoices.choices, verbose_name='Tipo')
    estado = models.CharField(
        max_length=20, choices=EstadoChoices.choices,
        default=EstadoChoices.PENDIENTE, verbose_name='Estado'
    )
    urgencia = models.CharField(
        max_length=10, choices=UrgenciaChoices.choices,
        default=UrgenciaChoices.NORMAL, verbose_name='Urgencia'
    )
    ...
    proceso = models.ForeignKey(
        ...
        related_name='tareas', verbose_name='Proceso'
    )
    usuario_responsable = models.ForeignKey(
        'accounts.User', on_delete=models.SET_NULL,
        blank=True, null=True, related_name='tareas_asignadas',
    ...
    def clean(self):
        if self.motivo_omision and not self.omitida:
            raise ValidationError({'motivo_omision': 'Solo aplica cuando la tarea está omitida.'})
        if self.fecha_completado and self.estado not in (
            self.EstadoChoices.COMPLETADA, self.EstadoChoices.GESTIONADO_EXTERNO
        ):
...
class TaskDeadlineConfig(models.Model):
    ...
    tipo_tarea = models.CharField(
        max_length=30, choices=Task.TipoChoices.choices,
        unique=True, verbose_name='Tipo de tarea'
    )
