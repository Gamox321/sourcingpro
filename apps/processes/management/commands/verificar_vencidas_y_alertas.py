...
class Command(BaseCommand):
    ...
    def _verificar_tareas_vencidas(self, now):
        ...
        self.stdout.write(
            f"  - {tareas_vencidas.count()} tareas marcadas como vencidas"
        )
    ...
    def _verificar_tareas_proximas_vencer(self, now):
        ...
        self.stdout.write(
            f"  - {tareas_proximas.count()} notificaciones de próximo vencimiento"
        )
    ...
    def _notificar_al_jefe_de_area(self, task):
        ...
        jefes = User.objects.filter(
            is_active=True, roles__nombre__in=["administrador", "jefatura", area]
        ).distinct()
