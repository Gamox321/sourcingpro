...
class AuditLog(models.Model):
    ...
    tabla_afectada = models.CharField(
        max_length=50, verbose_name="Tabla afectada", db_index=True
    )
    ...
    fecha_accion = models.DateTimeField(
        auto_now_add=True, verbose_name="Fecha de acción", db_index=True
    )
    usuario = models.ForeignKey(
        "accounts.User", on_delete=models.CASCADE, verbose_name="Usuario"
    )
    ...
    def __str__(self):
        return (
            f"{self.get_accion_display()} en {self.tabla_afectada}"
            f"#{self.id_entidad_afectada} ({self.fecha_accion.date()})"
        )
